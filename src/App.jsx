...
function CreateForm({ onCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setResult(null)
    try {
      // FIX: Your C# ShortenURL action expects a plain string 'longUrl'
      ...
      {result && (
        <div className={styles.resultBox}>
          ...
          <div className={styles.resultUrl}>
            {/* FIX: Matches C# property ShortCode */}
            <span className="mono">{result.shortCode}</span>
            <div className={styles.resultActions}>
              <button onClick={() => copy(result.shortCode)} className={styles.iconBtn} title="Copy">
                <Copy size={15} />
              </button>
            </div>
          </div>
      ...
      {urls.map(row => (
        <div key={row.id} className={styles.tableRow}>
          <div className={styles.urlCell}>
            {/* FIX: Matches C# property LongUrl */}
            <span className={styles.urlOrig}>{row.longUrl}</span>
          </div>
          <div className="mono" style={{ fontSize: 13 }}>
            {/* FIX: Matches C# property ShortCode */}
            <span className={styles.code}>{row.shortCode}</span>
          </div>
          <div style={{ fontSize: 12, color: 'var(--text3)' }}>
            <Clock size={12} style={{ marginRight: 4 }} />
            {/* FIX: Matches C# property createAt */}
            {fmt(row.createAt)}
          </div>
          <div className={styles.actions}>
            <button onClick={() => copy(row.shortCode)} className={styles.iconBtn} title="Copy code">
              <Copy size={14} />
            </button>
...
export default function App() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // FIX: Adjusted for plain array return if you aren't using a PagedList object yet
